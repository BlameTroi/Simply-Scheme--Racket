;; Diff this against ch24--spread.scm to see changes made for the
;; problem set of chapter 25.
...
;; Global state/constants for problems 25.1 and 25.12.

(define *total-rows* 200) ;; tested at 40 for 25.1, but later
                         ;; problems expect it to be 30
(define *total-cols* 26) ;; tested at 52 for 25.2, engine
                         ;; works but marshaling from external
                         ;; name such as aa17 to an internal
                         ;; cell id will require some parsing
                         ;; changes that I'm not going to
                         ;; bother with for this exercise.
                         ;; The point is internal and external
                         ;; are cleanly separated.
(define *default-cell-width* 10)
(define *display-rows* 20)
(define *display-cols* 6)

;; End global state additions.
...
;; An attempted at paging for 25.7.

(define (page-up . dontcare)
  (let ((row (id-row (screen-corner-cell-id))))
    (if (< (- row *display-rows*) 1)
      'do-nothing ;; not error
      (set-corner-row! (max 1 (- row *display-rows*))))))

(define (page-down . dontcare)
  (let ((row (id-row (screen-corner-cell-id))))
    (if (> (+ row *display-rows*) *total-rows*)
      'do-nothing
      (set-corner-row! (min *total-rows* (+ row *display-rows*))))))

(define (page-left . dontcare)
  (let ((col (id-column (screen-corner-cell-id))))
    (if (< (- col 3) 1)
      'do-nothing
      (set-corner-column! (max 1 (- col 3))))))

(define (page-right . dontcare)
  (let ((col (id-column (screen-corner-cell-id))))
    (if (< (+ col 3) *total-cols*)
      'do-nothing
      (set-corner-column! (min *total-cols* (+ col 3))))))
...
;; For 25.2 the column label needs to recognize the length of the
;; column name and adjust the width of the label when we roll
;; from x y z to aa ab ac.
...
;;        (equal? 'id (car x))))
;;
;; Changing cell id from list to vector for 25.5.
...
;; End Changing cell id from list to vector for 25.5.
...
;; For 25.2 add column names (I used doubling up on letters) and change
;; some of the parsing in the functions following to allow for a two
;; character (or more) column name.
(define alphabet
  '#(
